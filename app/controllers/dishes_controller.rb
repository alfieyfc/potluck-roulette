class DishesController < ApplicationController
  ...
  def create
    ...
    @dish.img_url = ENV['AWS_S3_ENDPOINT'] + @event.event_date.strftime("%y%m%d%H%M%s")+"-"+@event.id + "/" + h[:filename]

    s3 = Aws::S3::Resource.new
    s3.bucket(@event.event_date.strftime("%y%m%d%H%M%s")+"-"+@event.id).object(h[:filename]).upload_file(h[:file])
